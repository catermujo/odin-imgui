...
when im.BACKEND_WEBGL_ENABLED {
    ...
    GetBackendData :: proc() -> ^Data {
        ctx := im.GetCurrentContext()
        if ctx != nil {
            return cast(^Data)im.GetIO().BackendRendererUserData
        }
    ...
    RenderDrawData :: proc(draw_data: ^im.DrawData) {
        ...
        for n in 0 ..< draw_data.CmdListsCount {
            draw_list := (cast([^]^im.DrawList)draw_data.CmdLists.Data)[n]

            vtx_buffer_size := int(draw_list.VtxBuffer.Size) * size_of(im.DrawVert)
            idx_buffer_size := int(draw_list.IdxBuffer.Size) * size_of(im.DrawIdx)
            ...
            for cmd_i in 0 ..< draw_list.CmdBuffer.Size {
                ...
                if pcmd.UserCallback != nil {
                    ...
                    gl.BindTexture(gl.TEXTURE_2D, cast(gl.Texture)uintptr(im.DrawCmd_GetTexID(pcmd)))

                    gl.DrawElements(
                        gl.TRIANGLES,
                        int(pcmd.ElemCount),
                        size_of(im.DrawIdx) == 2 ? gl.UNSIGNED_SHORT : gl.UNSIGNED_INT,
                        cast(rawptr)uintptr(pcmd.IdxOffset * size_of(im.DrawIdx)),
                    )
                }
            }
        }
